...
extern void i2c_master_init(i2c_master_bus_handle_t *bus_handle, i2c_master_dev_handle_t *dev_handle);
...
{
   i2c_master_bus_handle_t b_handle;
   i2c_master_dev_handle_t d_handle;

  i2c_master_init(&b_handle,&d_handle);
...
{ 
   ...
   */
    ...
    ESP_ERROR_CHECK(    i2c_master_transmit(LCD_handle, data, len,  pdMS_TO_TICKS(1000))   );
...
void DFRobot_RGBLCD1602::setReg(uint8_t addr, uint8_t data)
{
    ...
    */
     uint8_t write_buf[2] = {addr, data};
        ESP_ERROR_CHECK(    i2c_master_transmit(RGB_handle, write_buf, sizeof(write_buf),  pdMS_TO_TICKS(1000))   );
}
...
void DFRobot_RGBLCD1602::printstr(const char *str)
{
        ...
        uint8_t col = 0;
        uint8_t row = 0;
        const uint8_t cols = _cols;
        const uint8_t rows = _rows;

        while(*str)
        {
                this->write((uint8_t)*str);
                str++;
                col++;
